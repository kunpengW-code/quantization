...
from vllm_ascend.utils import AscendDeviceType, COMPRESSED_TENSORS_METHOD
...
class TestAscendW8A8LinearMethod(TestBase):
    ...
    def test_apply_with_npu(self, mock_get_weight_prefetch_method):
        ...
        layer.aclnn_input_scale = torch.nn.Parameter(input_scale, requires_grad=False)
        layer.aclnn_input_scale_reciprocal = 1 / torch.nn.Parameter(input_scale, requires_grad=False)
        layer.aclnn_input_offset = torch.nn.Parameter(
            torch.empty(input_size, dtype=torch.int8).npu(), requires_grad=False
        ).to(layer.aclnn_input_scale.dtype)

        x = torch.randn(32, 128, dtype=torch.bfloat16).npu()
        bias = torch.randn(256, dtype=torch.float32).npu()
    ...
    @patch('torch_npu.npu_format_cast')
    def test_process_weights_after_loading_with_nz2_and_compressed_tensors(self,
                                                                           mock_npu_format_cast):
        layer = MagicMock()
