...
from vllm_ascend.quantization.methods.w4a4_flatquant import (
    ...
    pack_int4_weights, KRONECKER_QUANT_MAX_BATCH_SIZE, batched_kronecker_quant)
...
class TestW4A4FlatQuantDynamic(unittest.TestCase):
    ...
    def test_apply_with_npu(self):
        ...
        layer = nn.Module()
        layer.weight = nn.Parameter(torch.randint(-8, 7, (output_size, input_size), dtype=torch.int8).npu(), requires_grad=False)
        m, n = get_decompose_dim(input_size)
        layer.weight_scale = nn.Parameter(torch.ones(output_size, 1, dtype=torch.float32).npu() * 0.1, requires_grad=False)
        layer.weight_offset = nn.Parameter(torch.zeros(output_size, 1, dtype=torch.float32).npu(), requires_grad=False)
        layer.left_trans = nn.Parameter(torch.eye(m, dtype=torch.bfloat16).npu(), requires_grad=False)
        layer.right_trans = nn.Parameter(torch.eye(n, dtype=torch.bfloat16).npu(), requires_grad=False)
        layer.clip_ratio = nn.Parameter(torch.tensor([0.95], dtype=torch.float32).npu(), requires_grad=False)

        self.method.process_weights_after_loading(layer)
        x = torch.randn(batch_size, input_size, dtype=torch.bfloat16).npu()
