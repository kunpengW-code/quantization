...
class TestAscendW8A16LinearMethod(TestBase):
    ...
    def test_apply_with_npu(self):
        ...
        layer.weight = torch.nn.Parameter(torch.randint(-128, 127, (128, 256), dtype=torch.int8).npu(), requires_grad=False)
        layer.weight_scale = torch.nn.Parameter(torch.randn(256, dtype=torch.bfloat16).npu(), requires_grad=False)
        layer.weight_offset = torch.nn.Parameter(torch.randn(256, dtype=torch.bfloat16).npu(), requires_grad=False)
